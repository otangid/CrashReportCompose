package otang.id.lib.crashreport.activity
...
object CrashActivity {
    ...
    private fun getLastCrashTimestamp(context: Context) = context.getSharedPreferences(SHARED_PREFERENCES_FILE, Context.MODE_PRIVATE).getLong(SHARED_PREFERENCES_FIELD_TIMESTAMP, -1)

    private fun setLastCrashTimestamp(context: Context, timestamp: Long) = context.getSharedPreferences(SHARED_PREFERENCES_FILE, Context.MODE_PRIVATE).edit {
        putLong(SHARED_PREFERENCES_FIELD_TIMESTAMP, timestamp)
    }

    private fun guessErrorActivityClass(context: Context): Class<out Activity> = getErrorActivityClassWithIntentFilter(context) ?: DefaultErrorActivity::class.java
    ...
    private fun isStackTraceLikelyConflictive(t: Throwable?, activityClass: Class<out Activity>): Boolean {
        ...
        while (current != null) {
            if (current.stackTrace.any {
                    (it.className == "android.app.ActivityThread" && it.methodName == "handleBindApplication") || it.className == activityClass.name
                }) return true
    ...
    private fun guessRestartActivityClass(context: Context): Class<out Activity>? = getRestartActivityClassWithIntentFilter(context) ?: getLauncherActivity(context)
